    ...
    def setup(self):
        ...
        try:
            for port in self.port :
                self.ser.append(serial.Serial(port, self.burate, timeout=self.timeout))
...
# Modify plot_data function to return scatter plot objects
    def plot_data(self,datas, timestamps, color=None, label=None):
        self.plot.scatter(timestamps,datas,c= color,label = label,s= 100)
...
class Config:
    ...
    def build_login_ui(self):
        ...
        tk.Button(
            ...
            bg="#4CAF50", fg="white", width=10, command=self.do_login
